const rateLimit = require('express-rate-limit');
const RedisStore = require('rate-limit-redis');
const redisClient = require('../../../../shared/src/configurations/redis.configurations');

function expressRateLimiterMiddleware({
  endpoint,
  windowDurationInMinutes,
  requestLimit,
  statusCode,
  ErrorMessage,
}) {
  return rateLimit({
    windowMs: windowDurationInMinutes * 60 * 1000, // Convert minutes to milliseconds
    max: requestLimit, // Limit each IP to <requestLimit> requests per `window` (here, per 30 seconds)
    message: ErrorMessage,
    statusCode: statusCode,
    standardHeaders: true, // Return rate limit info in the `RateLimit-*` headers
    legacyHeaders: true, // Disable the `X-RateLimit-*` header

    // Redis store configuration to store the client ip along with the number of hits made by it in the given time window specified above
    // example keyname is like =>  keyname::<ip> and value would be the number of hits made in the given time window  6
    store: new RedisStore({
      sendCommand: (...args) => {
        return redisClient.sendCommand(args);
      },
      prefix: `EndPoint::/${endpoint}_IP`,
    }),
  });
}

module.exports.authSignUpMiddlewares = {
  expressRateLimiterMiddleware: expressRateLimiterMiddleware({
    endpoint: 'authentication/sign-up',
    windowDurationInMinutes: 0.5, // 30 seconds
    requestLimit: 2, // Limit each IP to 2 requests per 30 seconds
    statusCode: 429, // HTTP status code for rate limit exceeded
    ErrorMessage: 'Too many requests from your IP. Please try again later.',
  }),
};

module.exports.authLoginMiddlewares = {
  expressRateLimiterMiddleware: expressRateLimiterMiddleware({
    endpoint: 'authentication/login',
    windowDurationInMinutes: 0.5, // 30 seconds
    requestLimit: 2, // Limit each IP to 2 requests per 30 seconds
    statusCode: 429, // HTTP status code for rate limit exceeded
    ErrorMessage: 'Too many requests from your IP. Please try again later.',
  }),
};

module.exports.bookManageCreateBookMiddlewares = {
  expressRateLimiterMiddleware: expressRateLimiterMiddleware({
    endpoint: 'book-management/create-book',
    windowDurationInMinutes: 0.5, // 30 seconds
    requestLimit: 2, // Limit each IP to 2 requests per 30 seconds
    statusCode: 429, // HTTP status code for rate limit exceeded
    ErrorMessage: 'Too many requests from your IP. Please try again later.',
  }),
};

module.exports.bookManageDeleteBookMiddlewares = {
  expressRateLimiterMiddleware: expressRateLimiterMiddleware({
    endpoint: 'book-management/delete-book',
    windowDurationInMinutes: 0.5, // 30 seconds
    requestLimit: 2, // Limit each IP to 2 requests per 30 seconds
    statusCode: 429, // HTTP status code for rate limit exceeded
    ErrorMessage: 'Too many requests from your IP. Please try again later.',
  }),
};

module.exports.bookManageUpdateBookMiddlewares = {
  expressRateLimiterMiddleware: expressRateLimiterMiddleware({
    endpoint: 'book-management/update-book',
    windowDurationInMinutes: 0.5, // 30 seconds
    requestLimit: 2, // Limit each IP to 2 requests per 30 seconds
    statusCode: 429, // HTTP status code for rate limit exceeded
    ErrorMessage: 'Too many requests from your IP. Please try again later.',
  }),
};


module.exports.bookManageUpdateISBNMiddlewares = {
  expressRateLimiterMiddleware: expressRateLimiterMiddleware({
    endpoint: 'book-management/update-ISBN',
    windowDurationInMinutes: 0.5, // 30 seconds
    requestLimit: 2, // Limit each IP to 2 requests per 30 seconds
    statusCode: 429, // HTTP status code for rate limit exceeded
    ErrorMessage: 'Too many requests from your IP. Please try again later.',
  }),
};

module.exports.bookManageListBooksMiddlewares = {
  expressRateLimiterMiddleware: expressRateLimiterMiddleware({
    endpoint: 'book-management/list-books',
    windowDurationInMinutes: 0.5, // 30 seconds
    requestLimit: 2, // Limit each IP to 2 requests per 30 seconds
    statusCode: 429, // HTTP status code for rate limit exceeded
    ErrorMessage: 'Too many requests from your IP. Please try again later.',
  }),
};


module.exports.bookManageSearchBooksMiddlewares= {
  expressRateLimiterMiddleware: expressRateLimiterMiddleware({
    endpoint: 'book-management/search-books',
    windowDurationInMinutes: 0.5, // 30 seconds
    requestLimit: 2, // Limit each IP to 2 requests per 30 seconds
    statusCode: 429, // HTTP status code for rate limit exceeded
    ErrorMessage: 'Too many requests from your IP. Please try again later.',
  }),
};

module.exports.bookManageBookAvailabilityMiddlewares= {
  expressRateLimiterMiddleware: expressRateLimiterMiddleware({
    endpoint: 'book-management/book-availability',
    windowDurationInMinutes: 0.5, // 30 seconds
    requestLimit: 2, // Limit each IP to 2 requests per 30 seconds
    statusCode: 429, // HTTP status code for rate limit exceeded
    ErrorMessage: 'Too many requests from your IP. Please try again later.',
  }),
};

module.exports.borrowingManageTransactBookMiddlewares= {
  expressRateLimiterMiddleware: expressRateLimiterMiddleware({
    endpoint: 'BorrowingManagement/transact-book',
    windowDurationInMinutes: 0.5, // 30 seconds
    requestLimit: 2, // Limit each IP to 2 requests per 30 seconds
    statusCode: 429, // HTTP status code for rate limit exceeded
    ErrorMessage: 'Too many requests from your IP. Please try again later.',
  }),
};



module.exports.borrowingManageListBorrowedBooksMiddlewares= {
  expressRateLimiterMiddleware: expressRateLimiterMiddleware({
    endpoint: 'list/all-borrowed-books',
    windowDurationInMinutes: 0.5, // 30 seconds
    requestLimit: 2, // Limit each IP to 2 requests per 30 seconds
    statusCode: 429, // HTTP status code for rate limit exceeded
    ErrorMessage: 'Too many requests from your IP. Please try again later.',
  }),
};


module.exports.borrowingManageUserHistoryMiddlewares= {
  expressRateLimiterMiddleware: expressRateLimiterMiddleware({
    endpoint: 'list/user-specific-history',
    windowDurationInMinutes: 0.5, // 30 seconds
    requestLimit: 2, // Limit each IP to 2 requests per 30 seconds
    statusCode: 429, // HTTP status code for rate limit exceeded
    ErrorMessage: 'Too many requests from your IP. Please try again later.',
  }),
};

module.exports.notifManageNotifyUserMiddlewares = {
  expressRateLimiterMiddleware: expressRateLimiterMiddleware({
    endpoint: 'list/notify-user',
    windowDurationInMinutes: 0.5, // 30 seconds
    requestLimit: 2, // Limit each IP to 2 requests per 30 seconds
    statusCode: 429, // HTTP status code for rate limit exceeded
    ErrorMessage: 'Too many requests from your IP. Please try again later.',
  }),
};

module.exports.notifManageNotifyAllMiddlewares = {
  expressRateLimiterMiddleware: expressRateLimiterMiddleware({
    endpoint: 'list/notify-all',
    windowDurationInMinutes: 0.5, // 30 seconds
    requestLimit: 2, // Limit each IP to 2 requests per 30 seconds
    statusCode: 429, // HTTP status code for rate limit exceeded
    ErrorMessage: 'Too many requests from your IP. Please try again later.',
  }),
};

module.exports.notifManageNotifyUserGroupsMiddlewares = {
  expressRateLimiterMiddleware: expressRateLimiterMiddleware({
    endpoint: 'notify-User-Groups/:groupName',
    windowDurationInMinutes: 0.5, // 30 seconds
    requestLimit: 2, // Limit each IP to 2 requests per 30 seconds
    statusCode: 429, // HTTP status code for rate limit exceeded
    ErrorMessage: 'Too many requests from your IP. Please try again later.',
  }),
};